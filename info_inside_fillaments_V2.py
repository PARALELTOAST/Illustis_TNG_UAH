...
# Constructing the Lum Then Whim then fil grids
# then applying the fil-grids
...
del (tree)
######################################### Whim Grid Construction ###############################################
WHIM_Overden = np.zeros([num_cells, num_cells, num_cells])
Mass_grid = np.zeros([num_cells, num_cells, num_cells])

WHIM_xyz = np.array([x_pos_WHIM, y_pos_WHIM, z_pos_WHIM]).T

WHIM_Overden, WHIM_grid_edges= np.histogramdd(WHIM_xyz, bins = (x_bins, y_bins, z_bins), weights = mass_WHIM / (box_len/num_cells) ** 3 / mean_baryon_den)
