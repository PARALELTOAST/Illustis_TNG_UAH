...
print('Number of filaments and value at seg_x[0][1] and seg_y[0]:', len(seg_x),len(seg_x[0]), seg_x[0][1], seg_y[0])
...
def filament_func(box_len, num_cells, seg_x_array_new, seg_y_array_new, seg_z_array_new, fil_grid):
    fil_rad = 0.5 #1.0Mpc radius...Try other radii
    for n in tqdm.trange(len(seg_x_array_new)):
...
np.save('fil_grid_5MPC.npy', fil_grid)
...
for i in range(20):
    ...
    imageFilSlice = np.mean(fil_grid[gridIndexMin:gridIndexMax, :, :],axis=0)
    ####### plotting filaments. Filaments given by Daniela ##########
    #for j in range(len(seg_x)):
